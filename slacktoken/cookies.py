...
def get_cookies() -> typing.Dict[str, str]:
	with contextlib.ExitStack() as exit_stack:
		...
		if sys.platform == "win32":
			try:
				with cookie_database_path.open("rb"):
					pass
			except PermissionError:
				_LOGGER.debug("Cookie database is locked. Will attempt to shadow copy...")
				cookie_database_path = exit_stack.enter_context(slacktoken.windows.ShadowFile(cookie_database_path))
